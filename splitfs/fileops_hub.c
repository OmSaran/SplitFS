...
RETT_SETUID ALIAS_SETUID(INTF_SETUID) WEAK_ALIAS("_hub_SETUID");
RETT_SETUID  _hub_SETUID(INTF_SETUID);
...
{
	CHECK_RESOLVE_FILEOPS(_hub_);
        DEBUG_FILE("CALL: _hub_SETUID for (%d)\n", uid);

	return _hub_managed_fileops->SETUID(CALL_SETUID);
}
