...
RETT_SYSCALL_INTERCEPT _sfs_OPEN(INTF_SYSCALL) {
	...
	if(nvf->valid)
	{
		LOG("There is already a file open with that FD (%i)!\n", result);
		assert(0);
		END_TIMING(open_t, open_time);
		GLOBAL_UNLOCK_WR();
		*result = fd;
...
RETT_SYSCALL_INTERCEPT _nvp_REAL_CLOSE(int file, ino_t serialno, int async_file_closing, long* result) {
...
RETT_SYSCALL_INTERCEPT _sfs_CLOSE(INTF_SYSCALL) {
	...
	rc_res = _nvp_REAL_CLOSE(file, serialno, 0, result);
